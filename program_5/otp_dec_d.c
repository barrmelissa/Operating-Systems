...
char *strdecode(char* dataString, char* keyString){
	...
	for(i=2; dataString[i] != '\0'; i++){		
        ...
        ret[j] = chars[(di - ki + strlen(chars)) % strlen(chars)];
		j++;
    }
...
char* encode(char buffer[], char encryptedString[180000]){
	...
	if(buffer[i] == 'd'){
		...
		char *temp = strdecode(dataString, keyString);
		...
		fprintf(stderr, "SERVER DEC ERROR: Not d for encoding\n");
		//adds the 'd' to the begainning of the string so that when it is sent back it knows that it is incorrect and will terminate
		encryptedString[0] = 'd';
		encryptedString[1] = '\0';
		return encryptedString;
